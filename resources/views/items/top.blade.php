        ...
        @forelse($items as $item)
            <section class="item_list">
                ...
                @else
                    <p class="sell">出品中</p>
                @endif
                <p class="item_name">商品NO.{{$item->id}} | 出品者：{{$item->user->name}} | 出品日時：{{$item->created_at}}</p>
                <div class="flex">
                    @if($item->image !== '')
                        <a href="{{route('items.show',$item)}}" class="item_image"><img src="{{asset('storage/' . $item->image)}}"></a>
                    @else
